const Monitoring = (function(){
  ...
  function computeFeatures(){
    ...
    const meanJerk = jerkVals.length ? (jerkVals.reduce((a,b)=>a+b,0)/jerkVals.length) : 0;
    const gyros = gyroBuffer.map(s => mag3(s.gx||0, s.gy||0, s.gz||0));
    const gyroMean = gyros.length ? gyros.reduce((a,b)=>a+b,0)/gyros.length : 0;
  ...
  function ruleEngine(features, sens){
    ...
    const peakDetected = features.maxAcc >= acc_peak_thresh || features.peaks >= 2;
  ...
  function start(){
    ...
    deviceMotionHandler = (ev) => {
      ...
      if(ev.rotationRate) pushGyro(t, ev.rotationRate.alpha||0, ev.rotationRate.beta||0, ev.rotationRate.gamma||0);
      DOM.accelVal.textContent = `${(a.x||0).toFixed(2)}, ${(a.y||0).toFixed(2)}, ${(a.z||0).toFixed(2)}`;
    ...
    deviceOrientationHandler = (ev) => {
      DOM.orientVal.textContent = `alpha:${ev.alpha?.toFixed(0)} beta:${ev.beta?.toFixed(0)} gamma:${ev.gamma?.toFixed(0)}`;
    };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((pos)=> {
        ...
        DOM.locVal.textContent = `${pos.coords.latitude.toFixed(5)}, ${pos.coords.longitude.toFixed(5)}`;
      }, (err) => {
        console.warn(err);
  ...
  function triggerAnomaly({features, reason}){
    ...
    if (!DOM.countdownOverlay.classList.contains('hidden')) return;

    DOM.countdown.textContent = '10';
    DOM.countdownOverlay.classList.remove('hidden');
    countdownRemaining = 10;
    pendingEvent = {features, reason, timestamp: new Date().toISOString(), location: lastLocation};

    countdownTimer = setInterval(() => {
      countdownRemaining -= 1;
      DOM.countdown.textContent = countdownRemaining;
    ...
    function cancelHandler(){
      ...
      DOM.cancelBtn.removeEventListener('click', cancelHandler);
    }
  ...
  async function sendEventToServer(eventObj){
    const payload = {
      userId: null,
      eventType: (eventObj.reason && eventObj.reason.includes('inactivity')) ? 'inactivity' : 'fall',
      timestamp: eventObj.timestamp,
      location: eventObj.location ? {lat: eventObj.location.latitude, lng: eventObj.location.longitude} : {},
      simulated: false,
